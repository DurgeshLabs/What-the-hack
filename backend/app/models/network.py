...
from sqlalchemy import JSON, BigInteger, Boolean, CheckConstraint, DateTime, Enum, ForeignKey, Index, Integer, Numeric, String, Text, UniqueConstraint, Uuid, func
from sqlalchemy.dialects.postgresql import JSONB
from sqlalchemy.orm import Mapped, mapped_column

from app.db.base import Base

# JSONB on PostgreSQL, plain JSON elsewhere (SQLite in the HTTP test suite). DDL on PostgreSQL is unchanged.
...
class RawFlow(Base, TimestampMixin):
    ...
    extra_json: Mapped[dict[str, Any]] = mapped_column(JSONType, default=dict, server_default="{}", nullable=False)
...
class WindowFeature(Base, TimestampMixin):
    ...
    missing_fields_json: Mapped[list[str]] = mapped_column(JSONType, default=list, server_default="[]", nullable=False)
...
class ModelVersion(Base, TimestampMixin):
    ...
    metrics_json: Mapped[dict[str, Any]] = mapped_column(JSONType, default=dict, server_default="{}", nullable=False)
...
class Prediction(Base):
    ...
    explanation_json: Mapped[list[dict[str, Any]]] = mapped_column(JSONType, default=list, server_default="[]", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class Alert(Base, TimestampMixin):
    ...
    recommended_actions_json: Mapped[list[str]] = mapped_column(JSONType, default=list, server_default="[]", nullable=False)
...
class AuditLog(Base):
    ...
    metadata_json: Mapped[dict[str, Any]] = mapped_column(JSONType, default=dict, server_default="{}", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
