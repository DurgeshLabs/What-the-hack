...
from ai.inference.contract import FEATURE_NAMES, FEATURE_SCHEMA_VERSION, validate_features
...
def forecast(model: WorldModel, checkpoint: dict[str, Any], history: Sequence[dict[str, float]], k_steps: int = 5) -> dict[str, Any]:
    ...
    return {"risk_timeline": risks, "predicted_stages": stages, "peak_risk_window": peak, "peak_risk_stage": stages[peak], "top_contributing_features": explain_step(model, observed, peak, k_steps)}
